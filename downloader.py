...
def _task_resume_requested(task, default=False):
    task = task or {}
    return bool(task.get("resume_requested", default))
...
def _task_output_path_value(task, prefer_temp=False, default=""):
    ...
    primary_value = str(task.get(primary_key) or "").strip()
    if primary_value:
        return primary_value
    secondary_value = str(task.get(secondary_key) or "").strip()
    if secondary_value:
...
class DownloadManagerApp:
    ...
    def _log_m3u8_route_selected(self, task, item_id, media_url, source_site=None, fallback_urls=None):
        ...
        fallback_count = len(
            self._normalize_fallback_urls(
                fallback_urls if fallback_urls is not None else self._get_task_fallback_urls(task),
    ...
    def _eta_direct_media_text(self):
        return self._eta_or_status_text("eta_direct_media", "直接媒體下載")

    def _eta_found_media_text(self):
        return self._eta_or_status_text("eta_found_media", "已取得媒體網址")

    def _eta_found_stream_text(self):
        return self._eta_or_status_text("eta_found_stream", "已取得串流網址")

    def _eta_file_exists_text(self):
        return self._eta_or_status_text("eta_file_exists", "檔案已存在")

    def _message_file_exists_text(self):
        return t("msg_file_exists") if "msg_file_exists" in I18N_DICT.get(CURRENT_LANG, {}) else "檔案已存在"
    ...
    def _download_m3u8_with_ffmpeg(self, item_id, url, save_dir, is_mp3=False, referer="https://www.movieffm.net/", origin="https://www.movieffm.net", _unexpected_retry_done=False):
        task = self.tasks.get(item_id, {})
        ...
        for candidate_url in candidate_urls:
            ...
            if return_code == 15 and not _unexpected_retry_done:
                write_error_log(
    ...
    def _download_task_internal(self, url, item_id, save_dir, use_impersonate, is_mp3=False):
        ...
        if "gimy" in parsed_url.netloc and ("/detail/" in parsed_url.path or "/voddetail/" in parsed_url.path or "/voddetail2/" in parsed_url.path or "/vod/" in parsed_url.path):
            ...
            if self._is_gimy_movie_detail(entries):
                ...
                episode_entries = self._group_gimy_episode_entries(entries)
                primary = episode_entries[0]
                first_episode_url = primary["url"]
                first_episode_name = primary["full_name"]
                fallback_urls = primary.get("fallback_urls", [])
        ...
        if "facebook.com" in parsed_url.netloc and any(part in parsed_url.path for part in ("/reel/", "/watch/", "/videos/")):
            self._set_task_site_parsing_ui(item_id, "eta_site_facebook", "正在解析 Facebook 頁面...")
            c_req = get_curl_cffi_requests()
