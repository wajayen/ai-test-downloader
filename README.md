...
- HLS 與站台候選來源會集中進入共用派送流程，統一處理來源站、fallback URL、輸出檔名、安全檔名與路由記錄。
- `yt-dlp` 站台路由會集中管理格式選擇、瀏覽器 cookies、片段並行數、HTTP chunk size 與 MP4 合併策略。
...
- 搜尋到的可下載來源會優先檢查是否能取得直接影片或 HLS manifest，避免把無法解析的普通網頁加入下載任務。
...
- GitHub 同步點前需依照目前程式實際功能檢查並更新本 README，確認說明內容與新版功能一致。
