...
def _should_prefer_native_hls(url, task=None):
    ...
    if source_site == "hohoj":
        if any(marker in host for marker in ("ggjav", "cdnlab.live")):
            return True
        return False
    if source_site == "nnyy":
...
class DownloadManagerApp:
    ...
    def _is_incomplete_hls_video_artifact(self, task, output_path, expected_duration=None):
        ...
        try:
            ...
            resume_keys = []
            for candidate in (
                _task_url_value(task, fallback_url=_task_url_value(task)),
                self._get_task_source_page(task, fallback_url=_task_url_value(task)),
            ...
            resume_key = resume_keys[0] if resume_keys else self._get_task_resolved_url(task, fallback_url=_task_url_value(task))
            temp_root = self._get_stable_resume_base(_task_url_value(task), ext=ext, resume_key=resume_key)
    ...
    def _complete_if_output_exists(self, item_id):
        ...
        if self._can_accept_existing_output(task, item_id, explicit_output):
            ...
            self._set_task_named_column_text(item_id, "name", _output_name_from_path(explicit_output))
        ...
        for ext in possible_exts:
            ...
            if self._can_accept_existing_output(task, item_id, candidate_output):
                ...
                self._set_task_named_column_text(item_id, "name", _output_name_from_path(candidate_output))
    ...
    def _build_resume_output_identity_key(self, task, ext="mp4", save_dir=None, fallback_name="Video"):
        ...
        safe_name = _task_output_basename(task, fallback_name)
        if not safe_name:
            return ""
    ...
    def _download_direct_media_audio_with_ffmpeg(self, item_id, url, save_dir, referer=None, origin=None):
        task = self.tasks.get(item_id, {})
        safe_name = _task_output_basename(task, "Audio")
    ...
    def _download_mega_public_file(self, item_id, url, save_dir, is_mp3=False):
        ...
        if not is_folder_link and mega_client is not None:
            ...
            if not raw_name:
                raw_name = _task_display_name(task, fallback_url=url, default_is_mp3=is_mp3) or "mega_download.bin"
            ...
            if is_mp3:
                ...
                self._set_task_named_column_text(item_id, "name", _output_name_from_path(source_out_path))
    ...
    def _download_m3u8_with_ytdlp_native(self, item_id, url, save_dir, is_mp3=False, referer="https://www.movieffm.net/", origin="https://www.movieffm.net"):
        ...
        if actual_output:
            ...
            self._set_task_named_column_text(item_id, "name", _output_name_from_path(actual_output))
        final_output_path = _task_output_path_value(task, default=out_path) or out_path
        ...
        if self._complete_if_output_exists(item_id):
            write_error_log(
                ...
                item_id=item_id,
                source_site=_task_source_site_name(task) or None,
                output=_task_output_path_value(task, default=out_path) or out_path,
                bytes=self._get_existing_file_size(_task_output_path_value(task, default=out_path) or out_path),
            )
            return
        write_error_log(
    ...
    def _download_m3u8_with_ffmpeg(self, item_id, url, save_dir, is_mp3=False, referer="https://www.movieffm.net/", origin="https://www.movieffm.net", _unexpected_retry_count=0, _native_fallback_done=False):
        ...
        def _gimy_detail_rebuild_after_stream_failure(refresh_history, source_page_url, exc_obj):
            ...
            for attempted_page in (
                _task_url_value(task, fallback_url=""),
                source_page_url,
            ):
        ...
        for candidate_url in ffmpeg_candidate_urls:
            ...
            if return_code == 15 and not _native_fallback_done and _task_source_site_name(current_task) == "gimy":
                write_error_log(
                    ...
                    fallback_count=len(_task_fallback_urls_list(current_task)),
                    return_code=return_code,
                    state=current_state,
                    stop_reason=stop_reason,
                    recent_output=" | ".join(recent_lines)[:240],
    ...
    def resume_selected(self):
        for item_id in self._selected_task_ids():
            ...
            self._start_download_thread(
                _task_url_value(task),
                _task_display_name(task, fallback_url=_task_url_value(task), default_is_mp3=is_mp3),
    ...
    def _flush_task_resume_artifacts(self, task):
        temp_path = _task_output_path_value(task, prefer_temp=True)
        if not temp_path:
            return
    ...
    def _prepare_shutdown_resume_state(self):
        ...
        for item_id, task, proc in list(self._iter_active_process_handles()):
            media_url = _task_url_value(task)
            try:
    ...
    def _download_task_internal(self, url, item_id, save_dir, use_impersonate, is_mp3=False):
        ...
        safe_name = _task_output_basename(task, short_name)
        ...
        if is_direct_media:
            ...
            if _is_mixdrop_direct_media(url, direct_referer):
                ...
                warmup_referer = _task_url_value(task, fallback_url="https://www.movieffm.net/") or "https://www.movieffm.net/"
                try:
                    direct_session.get(
                        direct_referer,
                    ...
                    try:
                        ...
                        direct_session = None
            try:
                self._download_http_media(item_id, url, _task_output_path_value(task), headers=direct_headers, session=direct_session)
                return
            except Exception as e:
                if _is_mixdrop_direct_media(url, direct_referer):
                    raise
        ...
        if "hohoj.tv" in parsed_url.netloc and parsed_url.path.startswith("/video"):
            ...
            if stream_url:
                fallback_urls = [candidate for candidate in manifest_candidates[1:] if candidate != stream_url]
                _set_task_identity(name=page_title, source_site="hohoj", source_page=url, fallback_urls=fallback_urls)
                self._log_m3u8_route_selected(task, item_id, stream_url, source_site="hohoj", fallback_urls=fallback_urls)
                _download_manifest_with_site_strategy(
                    stream_url,
                    referer=embed_url,
                    origin=site_root,
            ...
            if media_url:
                ...
                _set_task_identity(name=page_title, source_site="hohoj", source_page=url, fallback_urls=fallback_urls)
                if is_mp3:
                    self._download_direct_media_audio_with_ffmpeg(item_id, media_url, save_dir, referer=embed_url, origin=site_root)
                else:
                    ext = os.path.splitext(urllib.parse.urlparse(media_url).path)[1] or ".mp4"
                    out_name = re.sub(r'[\\/:*?"<>|]+', "_", page_title).strip() or "video"
                    out_path = os.path.join(save_dir, out_name + ext)
                    self._download_http_media(
                        ...
                        media_url,
                        out_path,
                        headers={"Referer": embed_url, "Origin": site_root, "User-Agent": DEFAULT_USER_AGENT},
                    )
                return
            raise Exception("HoHoJ media URL missing")

        if "goodav17.com" in parsed_url.netloc and "/vr_html/" in parsed_url.path:
            self._set_task_parse_ui(item_id, key="eta_found_stream", fallback=self._ui_text("eta_found_stream", "已取得串流網址"))
            c_req = get_curl_cffi_requests()
            site_root = f"{parsed_url.scheme}://{parsed_url.netloc}"
            resp = c_req.get(
            ...
            direct_media_candidates = [
                candidate for candidate in candidate_urls
                if _looks_like_http_media_url(candidate)
                and "vr.goodav17.com/media/" in candidate.lower()
            ]
            stream_url = manifest_candidates[0] if manifest_candidates else ""
            media_url = direct_media_candidates[0] if direct_media_candidates else ""
            fallback_urls = []
            if stream_url:
                fallback_urls = [candidate for candidate in manifest_candidates[1:] if candidate != stream_url]
                _set_task_identity(name=page_title, source_site="goodav17", source_page=url, fallback_urls=fallback_urls)
                self._log_m3u8_route_selected(task, item_id, stream_url, source_site="goodav17", fallback_urls=fallback_urls)
                _download_manifest_with_site_strategy(
                    stream_url,
                    referer=url,
                    origin=site_root,
