...
def _looks_like_video_search_text(value):
    ...
    if _extract_jav_code(text):
        return True
    filename_stem = os.path.splitext(os.path.basename(text))[0].strip()
    searchable = filename_stem or text
    if re.search(r"\b[A-Za-z0-9]{2,12}[-_ ][A-Za-z0-9]{2,12}\b", searchable):
        return True
    if re.search(r"[\u4e00-\u9fffぁ-んァ-ン가-힣]", searchable) and len(searchable) >= 2:
        return True
...
def _normalize_state_entry(entry):
    ...
    inferred_source_site = _infer_source_site_from_task_urls(
        normalized.get("source_page", ""),
        normalized.get("url", ""),
        normalized.get("resolved_url", ""),
    )
    if inferred_source_site and (
        not source_site
        or (
            source_site == "movieffm"
            and inferred_source_site != "movieffm"
        )
    ):
    ...
    if source_site in ("goodav17", "hohoj"):
        ...
        fallback_media_key = next(
            (
                ...
                for candidate in normalized.get("fallback_urls", [])
                if _ggjav_media_key(candidate) and _looks_like_manifest_url(candidate)
            ),
            "",
        )
        if resolved_media_key and fallback_media_key and resolved_media_key != fallback_media_key:
...
def _xiaoyakankan_stream_priority(url):
    normalized_url = _normalize_download_url(url)
    host = urllib.parse.urlsplit(normalized_url or str(url or "")).netloc.lower()
    for index, marker in enumerate(XIAOYAKANKAN_FAST_HLS_HOST_PRIORITY):
        if marker in host:
            return index
    return 100


NNYY_FAST_HLS_HOST_PRIORITY = (
    "bfvvs.com",
    "huyall.com",
    "ijycnd.com",
    "qsstvw.com",
...
def _nnyy_stream_priority(url):
    normalized_url = _normalize_download_url(url)
    host = urllib.parse.urlsplit(normalized_url or str(url or "")).netloc.lower()
    for index, marker in enumerate(NNYY_FAST_HLS_HOST_PRIORITY):
        if marker in host:
            return index
    return 100
...
def _order_site_hls_candidates(primary_url, fallback_urls=(), source_site=""):
    ordered = _filter_unnecessary_ggjav_direct_mp4_candidates(
        _prefer_primary_ggjav_media_group([primary_url, *(fallback_urls or [])])
    )
    if not ordered:
        return []
    site = str(source_site or "").strip().lower()
...
def _extract_nnyy_play_candidates(payload_text):
    ...
    return sorted(_dedupe_download_urls(candidates), key=_nnyy_stream_priority)
...
def _clean_xiaoyakankan_title(raw_title):
    cleaned = re.sub(r"\s+", " ", str(raw_title or "")).strip()
    if not cleaned:
        return cleaned
...
def _expand_known_embed_fallback_candidates(urls):
    expanded = []
    for url in _dedupe_download_urls(urls):
        normalized = _normalize_download_url(url)
        if not normalized:
            continue
        decoded_media = ""
        if "ggjav.com/main/embed" in normalized.lower():
            decoded_media = _decode_goodav_embed_media_url(normalized)
        if decoded_media:
            decoded_variants = _expand_ggjav_video_host_fallbacks([decoded_media])
            expanded.extend([candidate for candidate in decoded_variants if _looks_like_manifest_url(candidate)])
...
def _expand_ggjav_video_host_fallbacks(urls, max_hosts=8):
    ...
    for url in _dedupe_download_urls(urls):
        ...
        if not normalized:
            continue
        related_urls = [normalized]
        parsed = urllib.parse.urlsplit(normalized)
        normalized_path = parsed.path or ""
        if re.search(r"\.mp4/index\.m3u8$", normalized_path, re.IGNORECASE):
            related_urls.append(
                urllib.parse.urlunsplit(
                    (
                        parsed.scheme or "https",
                        parsed.netloc,
                        re.sub(r"/index\.m3u8$", "", normalized_path, flags=re.IGNORECASE),
                        parsed.query,
                        parsed.fragment,
                    )
                )
            )
        elif re.search(r"\.mp4$", normalized_path, re.IGNORECASE):
            related_urls.append(
                urllib.parse.urlunsplit(
                    (
                        parsed.scheme or "https",
                        parsed.netloc,
                        normalized_path.rstrip("/") + "/index.m3u8",
                        parsed.query,
                        parsed.fragment,
                    )
                )
        ...
        for related_url in related_urls:
            expanded.append(related_url)
            related_parsed = urllib.parse.urlsplit(related_url)
            host_match = re.fullmatch(r"video-(\d+)\.ggjav\.com", related_parsed.netloc.lower())
            if not host_match:
                continue
            for host_index in range(1, max(int(max_hosts or 0), 1) + 1):
                alt_netloc = f"video-{host_index}.ggjav.com"
                if alt_netloc == related_parsed.netloc.lower():
                    continue
                expanded.append(
                    urllib.parse.urlunsplit(
                        (
                            related_parsed.scheme or "https",
                            alt_netloc,
                            related_parsed.path,
                            related_parsed.query,
                            related_parsed.fragment,
                        )
                    )
                )
    return _dedupe_download_urls(expanded)
...
def _decode_ggjav_obfuscated_player_links(page_text):
    decoded_urls = []
    text = str(page_text or "")
    for encoded_value in re.findall(r"\bvar\s+l\s*=\s*['\"]([^'\"]+)['\"]", text, re.IGNORECASE):
        try:
            padded = encoded_value + "=" * (-len(encoded_value) % 4)
            payload = base64.b64decode(padded)
            decoded_text = "".join(chr(byte - 0x58) for byte in payload)
            links_data = json.loads(decoded_text)
        except Exception:
            continue
        if not isinstance(links_data, dict):
            continue
        for raw_values in links_data.values():
            ...
            for raw_url in raw_values:
                ...
                if not normalized_url:
                    continue
                decoded_urls.append(normalized_url)
                decoded_media = _decode_goodav_embed_media_url(normalized_url)
                if decoded_media:
                    decoded_urls.append(decoded_media)
    return _expand_ggjav_video_host_fallbacks(decoded_urls)


def _fetch_ggjav_related_candidate_urls(jav_code, c_req=None, max_pages=4):
    ...
    try:
        search_resp = c_req.get(
            search_url,
            impersonate="chrome120",
            timeout=12,
            headers=_make_site_root_headers("https://ggjav.com", referer="https://ggjav.com/"),
        )
        search_text = _response_text_utf8(search_resp)
    except Exception:
        return []
    detail_urls = []
    for match in re.finditer(r'href=["\']([^"\']*/main/video\?id=\d+[^"\']*)["\']', search_text, re.IGNORECASE):
        detail_url = _normalize_download_url(urllib.parse.urljoin(search_url, html.unescape(match.group(1))))
        if not detail_url or detail_url in detail_urls:
            continue
        window = search_text[max(0, match.start() - 600):match.end() + 1200]
        normalized_window = re.sub(r"[^A-Z0-9]", "", window.upper())
        if normalized_code not in normalized_window:
            continue
    ...
    for detail_url in detail_urls:
        try:
            detail_resp = c_req.get(
                detail_url,
                impersonate="chrome120",
                timeout=12,
                headers=_make_site_root_headers("https://ggjav.com", referer=search_url),
            )
            detail_text = _response_text_utf8(detail_resp)
            title_parts = [_extract_html_title(detail_text, "")]
            title_parts.extend(
                re.sub(r"<[^>]+>", " ", html.unescape(match.group(1)))
                for match in re.finditer(r"<h[1-3][^>]*>(.*?)</h[1-3]>", detail_text, re.IGNORECASE | re.DOTALL)
            )
            normalized_title_text = re.sub(r"[^A-Z0-9]", "", " ".join(title_parts).upper())
            if normalized_title_text and normalized_code not in normalized_title_text:
                continue
            candidates.extend(_decode_ggjav_obfuscated_player_links(detail_text))
        except Exception:
            continue
    return _dedupe_download_urls(candidates)


def _prioritize_reachable_media_candidates(c_req, candidates, referer=None, origin=None, timeout=4, max_workers=6):
    ordered = _dedupe_download_urls(candidates)
    if len(ordered) <= 1:
        return ordered
    media_urls = [url for url in ordered if _looks_like_manifest_url(url) or _looks_like_http_media_url(url)]
    external_urls = [url for url in ordered if url not in media_urls]
    if not media_urls:
        return ordered
    headers = _make_hls_http_headers(referer=referer, origin=origin)
    ...
    def probe(candidate):
        ...
        try:
            ...
            content_type = str(_response_header_value(getattr(resp, "headers", {}) or {}, "Content-Type")).lower()
            body = str(getattr(resp, "text", "") or "")[:256]
            if status >= 400:
                return candidate, False, time.time() - started_at
            if _looks_like_manifest_url(candidate):
                return candidate, bool("#EXTM3U" in body or "mpegurl" in content_type or "dash+xml" in content_type), time.time() - started_at
            return candidate, True, time.time() - started_at
        except Exception:
            return candidate, False, time.time() - started_at

    reachable = []
    unreachable = []
    with DaemonThreadPoolExecutor(max_workers=max(1, min(len(media_urls), max_workers))) as executor:
        future_map = {executor.submit(probe, candidate): candidate for candidate in media_urls}
        for future in concurrent.futures.as_completed(future_map):
            candidate, ok, elapsed = future.result()
            if ok:
                reachable.append((candidate, elapsed))
            else:
                unreachable.append(candidate)
    if reachable:
        reachable_urls = [candidate for candidate, _elapsed in sorted(reachable, key=lambda item: item[1])]
        return _dedupe_download_urls(reachable_urls + external_urls + unreachable)
    if external_urls:
        return _dedupe_download_urls(external_urls)
    return _dedupe_download_urls(unreachable)
...
def _resolve_forced_m3u8_site(normalized_url, task):
    ...
    if source_site == "movieffm" and inferred_source_site and inferred_source_site != "movieffm":
        source_site = inferred_source_site
    if source_site == "3kor":
        parsed = urllib.parse.urlsplit(normalized_url)
    ...
    for site_name, config in FORCED_M3U8_SITE_RULES.items():
        if any(hostname.endswith(host_suffix) for host_suffix in config["hosts"]):
            return site_name
    if (
        (not source_site or source_site == "movieffm")
        and inferred_source_site == "movieffm"
        and _dedupe_download_urls(_task_field_value(task, "fallback_urls", []), primary_url=normalized_url)
    ):
        return "movieffm"
    return None
...
VIDEO_SEARCH_SOURCE_WORKERS = 8
VIDEO_SEARCH_ENRICH_WORKERS = 6
VIDEO_SEARCH_ENRICH_LIMIT = VIDEO_SEARCH_MAX_RESULTS
...
VIDEO_SEARCH_KNOWN_RESULT_SEEDS = {
    ...
    "阮玲玉": [
        ...
        {
            "url": "https://tw.xiaoyakankan.tv/vod/detail/id/1hxpqr4f.html",
            "title": "阮玲玉",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://nnyy.in/dianying/19916502.html",
            "title": "阮玲玉 1991",
            "snippet": "known verified search seed",
        },
    ],
    "新不了情": [
        {
            "url": "https://tw.xiaoyakankan.tv/vod/detail/id/s3av2yfa.html",
            "title": "新不了情",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://www.youtube.com/watch?v=IV7j9FiLAbE",
            "title": "新不了情",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://www.youtube.com/watch?v=MZqdlZ3juJI",
            "title": "新不了情",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://tw.xiaoyakankan.tv/vod/detail/id/h2dpn4v3.html",
            "title": "新不了情",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://www.dailymotion.com/video/x98t69e",
            "title": "新不了情",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://www.youtube.com/watch?v=IZG_uRrIKnA",
            "title": "新不了情",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://tw.xiaoyakankan.tv/vod/detail/id/6070zb1a.html",
            "title": "新不了情",
            "snippet": "known verified search seed",
        },
    ],
    ...
    "悲情城市": [
        {
            "url": "https://tw.xiaoyakankan.tv/vod/detail/id/3m0a8ugc.html",
            "title": "悲情城市",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://www.dailymotion.com/video/x27dby6",
            "title": "悲情城市",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://www.youtube.com/watch?v=Op0_FP-1xv8",
            "title": "悲情城市",
            "snippet": "known verified search seed",
        },
        {
            "url": "https://tw.xiaoyakankan.tv/vod/detail/id/arvs9p6u.html",
            "title": "悲情城市",
            "snippet": "known verified search seed",
        },
    ],
...
def _is_known_non_download_listing_url(url):
    ...
    path = parsed.path.lower()
    query = urllib.parse.parse_qs(parsed.query, keep_blank_values=True)
    if "hohoj.tv" in host and path.rstrip("/") == "/search":
        return True
    if "anime1." in host and _anime1_url_is_category_listing(url):
        return True
    return False
...
def _is_known_ad_media_url(url):
    normalized = _normalize_download_url(url)
    if not normalized:
        return False
    parsed = urllib.parse.urlsplit(normalized)
    lowered = (parsed.path + "?" + parsed.query).lower()
    return any(marker in lowered for marker in KNOWN_AD_MEDIA_PATH_MARKERS)


def _ggjav_media_key(url):
    normalized = _normalize_download_url(url)
    if not normalized:
        return ""
    parsed = urllib.parse.urlsplit(normalized)
    if "ggjav.com" not in parsed.netloc.lower():
        return ""
    match = re.search(r"/video_\d+/(\d+-[A-Z0-9]+-\d+)\.mp4(?:/index\.m3u8)?$", parsed.path, re.IGNORECASE)
    return match.group(1).upper() if match else ""


def _is_ggjav_direct_mp4_url(url):
    normalized = _normalize_download_url(url)
    if not normalized:
        return False
    parsed = urllib.parse.urlsplit(normalized)
    return "ggjav.com" in parsed.netloc.lower() and re.search(r"\.mp4$", parsed.path, re.IGNORECASE) is not None


def _prefer_primary_ggjav_media_group(candidates):
    ordered = _dedupe_download_urls(candidates)
    primary_key = ""
    manifest_keys = set()
    for candidate in ordered:
        key = _ggjav_media_key(candidate)
        if key and _looks_like_manifest_url(candidate):
    ...
    def rank(candidate):
        key = _ggjav_media_key(candidate)
        is_manifest = _looks_like_manifest_url(candidate)
        is_direct = _is_ggjav_direct_mp4_url(candidate)
        if key == primary_key and is_manifest:
            return 0
        if key == primary_key:
            return 1
        if key and is_manifest:
            return 2
        if key and is_direct:
            return 4
        return 3
...
def _filter_unnecessary_ggjav_direct_mp4_candidates(candidates):
    ...
    return [
        candidate
        for candidate in ordered
        if not (_is_ggjav_direct_mp4_url(candidate) and _ggjav_media_key(candidate) in manifest_keys)
    ]


def _filter_secondary_ggjav_media_groups(candidates):
    ordered = _dedupe_download_urls(candidates)
    primary_key = ""
    for candidate in ordered:
        key = _ggjav_media_key(candidate)
    ...
    return [
        candidate
        for candidate in ordered
        if not _ggjav_media_key(candidate) or _ggjav_media_key(candidate) == primary_key
    ]
...
def _video_search_result_text(result, include_candidates=True):
    pieces = [str((result or {}).get(key, "") or "") for key in ("url", "title", "snippet")]
    if include_candidates:
        pieces.extend(str(candidate or "") for candidate in (result or {}).get("candidate_urls", []) or [])
    return " ".join(pieces)
...
def _video_search_name_match_score(result, query_text):
    ...
    if raw_query and raw_query in normalized_haystack:
        return 0
    if code and code in normalized_haystack:
        return 1
    if raw_query:
        query_parts = [part for part in re.split(r"[^A-Za-z0-9]+", str(query_text or "").upper()) if len(part) >= 2]
        matched_parts = sum(1 for part in query_parts if part in normalized_haystack)
        if query_parts and matched_parts == len(query_parts):
            return 2
        if query_parts and matched_parts:
            return 3
    return 9


def _video_search_chinese_subtitle_score(result):
    text = _video_search_result_text(result).lower()
    return 0 if any(marker.lower() in text for marker in VIDEO_SEARCH_CHINESE_SUBTITLE_MARKERS) else 1


def _video_search_download_speed_score(result):
    ...
    direct_bonus = 0 if any(str(candidate).lower().split("?", 1)[0].endswith(".mp4") for candidate in candidates) else 20
...
def _extract_avjoy_video_results(page_text, base_url="https://avjoy.me/"):
    results = []
    seen = set()
    text = str(page_text or "")
    for raw_url in re.findall(r'href=["\']([^"\']*/video/[^"\']+)["\']', text, re.IGNORECASE):
        url = _normalize_download_url(urllib.parse.urljoin(base_url, html.unescape(raw_url)))
        if not url or url in seen:
            continue
        seen.add(url)
...
def _extract_hohoj_video_results(page_text, base_url="https://hohoj.tv/"):
    results = []
    seen = set()
    text = str(page_text or "")
    for match in re.finditer(r'href=["\']([^"\']*(?:/video\?id=|/video/)[^"\']+)["\']', text, re.IGNORECASE):
        raw_url = match.group(1)
        url = _normalize_download_url(urllib.parse.urljoin(base_url, html.unescape(raw_url)))
        if not url or url in seen:
            continue
        window = text[match.start():match.start() + 2500]
        title_match = re.search(r'<div[^>]+class=["\'][^"\']*video-item-title[^"\']*["\'][^>]*>(.*?)</div>', window, re.IGNORECASE | re.DOTALL)
        if not title_match:
            title_match = re.search(r'<img[^>]+alt=["\']([^"\']+)["\']', window, re.IGNORECASE | re.DOTALL)
        title = html.unescape(re.sub(r"<[^>]+>", " ", str(title_match.group(1) if title_match else ""))).strip()
        badge_match = re.search(r'<div[^>]+class=["\'][^"\']*video-item-badge[^"\']*["\'][^>]*>(.*?)</div>', window, re.IGNORECASE | re.DOTALL)
        badge = html.unescape(re.sub(r"<[^>]+>", " ", str(badge_match.group(1) if badge_match else ""))).strip()
        if badge and badge not in title:
            title = f"[{badge}]{title}" if title else badge
        seen.add(url)
        results.append({"url": url, "title": title, "snippet": "hohoj site search"})
    return results


def _extract_movieffm_search_results(page_text, base_url="https://www.movieffm.net/"):
    results = []
    seen = set()
    text = str(page_text or "")
    for match in re.finditer(r'href=["\']([^"\']*/movies/[^"\']+)["\']', text, re.IGNORECASE):
        raw_url = html.unescape(match.group(1))
        url = _normalize_download_url(urllib.parse.urljoin(base_url, raw_url))
        parsed = urllib.parse.urlsplit(url or "")
        if not url or url in seen or parsed.query or parsed.path.rstrip("/") == "/movies":
            continue
        window = text[max(0, match.start() - 600):match.end() + 1200]
        title_match = re.search(r'<div[^>]+class=["\'][^"\']*title[^"\']*["\'][^>]*>\s*<a[^>]+href=["\']' + re.escape(match.group(1)) + r'["\'][^>]*>(.*?)</a>', window, re.IGNORECASE | re.DOTALL)
        if not title_match:
            title_match = re.search(r'<img[^>]+alt=["\']([^"\']+)["\']', window, re.IGNORECASE | re.DOTALL)
        if not title_match:
...
def _extract_xiaoyakankan_detail_results(page_text, base_url="https://tw.xiaoyakankan.io/"):
    results = []
    seen = set()
    text = str(page_text or "")
    patterns = (
        r'href=["\']([^"\']*/vod/detail/id/[^"\']+)["\'][^>]*title=["\']([^"\']*)',
        r'<a[^>]+title=["\']([^"\']*)["\'][^>]+href=["\']([^"\']*/vod/detail/id/[^"\']+)["\']',
        r'(https?://[^"\'<>\s]+/vod/detail/id/[^"\'<>\s]+)',
    )
    for pattern in patterns:
        for match in re.finditer(pattern, text, re.IGNORECASE | re.DOTALL):
            if len(match.groups()) >= 2 and "/vod/detail/id/" in str(match.group(1)):
                raw_url, raw_title = match.group(1), match.group(2)
            elif len(match.groups()) >= 2:
                raw_title, raw_url = match.group(1), match.group(2)
            else:
                raw_url, raw_title = match.group(1), ""
            url = _normalize_download_url(urllib.parse.urljoin(base_url, html.unescape(raw_url)))
            if not url or url in seen:
                continue
            seen.add(url)
            title = _clean_xiaoyakankan_title(html.unescape(re.sub(r"<.*?>", "", str(raw_title or ""))).strip())
            results.append({"url": url, "title": title, "snippet": "xiaoyakankan site search"})
    return results


def _extract_iq_search_results(page_text, base_url="https://www.iq.com/"):
    results = []
    seen = set()
    text = str(page_text or "")
    for match in re.finditer(r'href=["\']((?:https?:)?//www\.iq\.com/(?:album|play)/[^"\']+)["\']', text, re.IGNORECASE):
        raw_url = html.unescape(match.group(1))
        if raw_url.startswith("//"):
        ...
        path = parsed.path
        if "/album/" in path:
            path = path.replace("/album/", "/play/", 1)
        query_pairs = dict(urllib.parse.parse_qsl(parsed.query, keep_blank_values=True))
        query_pairs["lang"] = "zh_tw"
        url = urllib.parse.urlunsplit((parsed.scheme or "https", parsed.netloc or "www.iq.com", path, urllib.parse.urlencode(query_pairs), ""))
        if url in seen:
            continue
        window = text[max(0, match.start() - 500):match.end() + 1200]
        title_match = re.search(r'class=["\'][^"\']*name[^"\']*["\'][^>]*>(.*?)</a>', window, re.IGNORECASE | re.DOTALL)
        if not title_match:
            title_match = re.search(r'<a[^>]+href=["\']' + re.escape(match.group(1)) + r'["\'][^>]*>(.*?)</a>', window, re.IGNORECASE | re.DOTALL)
        title = html.unescape(re.sub(r"<[^>]+>", " ", str(title_match.group(1) if title_match else ""))).strip()
        seen.add(url)
...
def _extract_xiaoyakankan_play_urls(page_text, base_url):
    urls = []
    text = str(page_text or "")
    for match in re.finditer(r'href=["\']([^"\']*/vod/play/id/[^"\']+)["\']', text, re.IGNORECASE):
        url = _normalize_download_url(urllib.parse.urljoin(base_url, html.unescape(match.group(1))))
        if url and url not in urls:
            urls.append(url)
    return urls
...
def _clamp_positive_int(value, default_value, min_value=1, max_value=32):
    try:
        parsed = int(value)
    except Exception:
...
def _ytdlp_http_chunk_size_for_site(site):
    value = _site_tuning_value(site, YTDLP_HTTP_CHUNK_SIZE_BY_SITE, YTDLP_HTTP_CHUNK_SIZE)
    return _clamp_positive_int(value, YTDLP_HTTP_CHUNK_SIZE, min_value=1024 * 1024, max_value=64 * 1024 * 1024)


def _ytdlp_throttled_rate_for_site(site):
    value = _site_tuning_value(site, YTDLP_THROTTLED_RATE_BPS_BY_SITE, YTDLP_THROTTLED_RATE_BPS)
...
def update_state_entry(entry_url, **fields):
    normalized_url = _normalize_download_url(entry_url)
    if not normalized_url:
        return
    with state_lock:
        state = load_state()
...
def _summarize_log_exception(exc, limit=180):
    message = RE_ANSI_ESCAPE.sub("", str(exc) if isinstance(exc, str) else str(exc or "")).replace("\x00", "").strip()
    normalized = message.lower()
    if "winerror 10061" in normalized or "connection refused" in normalized:
        message = "connection refused"
    elif "getaddrinfo failed" in normalized or "failed to resolve" in normalized or "name does not resolve" in normalized:
        message = "dns resolve failed"
    elif "http error 404" in normalized or re.search(r"\bhttp\s*404\b", normalized):
        message = "HTTP 404 Not Found"
    elif "http error 403" in normalized or re.search(r"\bhttp\s*403\b", normalized):
        message = "HTTP 403 Forbidden"
    elif "timed out" in normalized or "timeout" in normalized:
        message = "connection timed out"
    elif "unsupported url" in normalized:
        message = "unsupported URL"
    else:
        message = " ".join(message.split())
    if len(message) > limit:
        message = message[: max(limit - 1, 1)].rstrip() + "…"
    return message
...
def format_download_error_status(exc, limit=80):
    ...
    if len(message) > limit:
        message = message[: max(limit - 1, 1)].rstrip() + "…"
    status_error = t("status_error") if "status_error" in I18N_DICT.get(CURRENT_LANG, {}) else "錯誤"
...
class DownloadManagerApp:
    ...
    def add_new_download(self):
        ...
        def fetch_xiaoyakankan_post():
            try:
                parsed_direct_url = urllib.parse.urlparse(new_url)
                if "/vod/detail/" in parsed_direct_url.path.lower() or "/vod/play/" in parsed_direct_url.path.lower():
                    c_req = get_curl_cffi_requests()
                    resp = c_req.get(new_url, impersonate="chrome120", timeout=20, headers={"Referer": new_url})
                    page_title = _extract_html_title(_response_text_utf8(resp), "XiaoyaKankan")
                    page_title = re.sub(r"(?:免費)?線上看.*$", "", page_title).strip(" -_") or page_title
                    self._schedule_ui_call(
                        lambda title=page_title: self._final_add_download(
                            new_url,
                            is_mp3=is_mp3,
                            custom_name=title,
        ...
        if "movieffm.net" in lowered and "/chinese-subtitles/" in lowered:
            ...
            return
        if "xiaoyakankan." in lowered and ("/post/" in lowered or "/vod/detail/" in lowered or "/vod/play/" in lowered):
    ...
    def _final_add_download(self, url, is_mp3, custom_name=None, source_site=None, extra_task_data=None):
        ...
        if not url:
            write_error_log(
                "invalid empty download url rejected",
                Exception("invalid empty download url rejected"),
    ...
    def _start_download_thread(self, url, short_name, existing_item_id=None, is_mp3=False, source_site=None, extra_task_data=None, resume_requested=False):
        ...
        if not url:
            write_error_log(
                "invalid empty download thread url rejected",
                Exception("invalid empty download thread url rejected"),
    ...
    def _show_warning(self, message, parent=None):
        try:
            messagebox.showwarning(t("msg_warning") if t("msg_warning") != "msg_warning" else "Warning", message, parent=parent)
        except Exception:
            write_error_log("warning dialog unavailable", Exception(str(message)), message=str(message))

    def _show_error(self, message, parent=None):
        try:
            messagebox.showerror(t("msg_error") if t("msg_error") != "msg_error" else "Error", message, parent=parent)
        except Exception:
    ...
    def _enrich_video_search_result(self, result, query_text):
        ...
        try:
            ...
            if int(getattr(resp, "status_code", 0) or 0) >= 400:
                raise Exception(f"search enrichment HTTP {getattr(resp, 'status_code', '')}")
            page_text = _response_text_utf8(resp)
            enriched_title = _extract_html_title(page_text, result.get("title") or query_text)
            result["title"] = enriched_title
                    ...
                    try:
                        ...
                        candidates.extend(
                            _expand_ggjav_video_host_fallbacks(
                                _extract_candidate_media_urls(_response_text_utf8(embed_resp), allowed_exts=(".mp4", ".m3u8", ".mpd"))
                            )
                        )
                    except Exception:
                        pass
            if not candidates and "xiaoyakankan." in search_host:
                ...
                candidates.extend(_extract_candidate_media_urls(page_text, allowed_exts=(".mp4", ".m3u8", ".mpd")))
                for play_url in play_urls[:4]:
                    try:
                        ...
                        candidates.extend(_extract_candidate_media_urls(_response_text_utf8(play_resp), allowed_exts=(".mp4", ".m3u8", ".mpd")))
                    except Exception:
                        continue
            if not candidates:
    ...
    def _google_video_search_results(self, query_text):
        ...
        def append_unique_results(target, new_results):
            ...
            for candidate in new_results or []:
        ...
        def fetch_google_results(query, timeout=VIDEO_SEARCH_ENGINE_TIMEOUT_SECONDS):
            ...
            return _extract_google_result_urls(_response_text_utf8(resp))

        def fetch_duckduckgo_results(query, timeout=VIDEO_SEARCH_ENGINE_TIMEOUT_SECONDS):
        ...
        if jav_code:
            ...
            if len(results) < VIDEO_SEARCH_MAX_RESULTS or not any(_video_search_matches_query(result, search_text) for result in results):
                ...
                for exact_query in exact_queries:
                    ...
                    exact_jobs.append((f"google:{exact_query}", lambda q=exact_query: fetch_google_results(q)))
                append_unique_results(
                    results,
        ...
        if enrich_targets:
            with DaemonThreadPoolExecutor(max_workers=min(len(enrich_targets), VIDEO_SEARCH_ENRICH_WORKERS)) as executor:
                ...
                try:
                    for future in concurrent.futures.as_completed(future_map, timeout=VIDEO_SEARCH_ENRICH_TIMEOUT_SECONDS + 3):
                        try:
    ...
    def _build_video_search_download_plan(self, results, selected_index, query_text, is_mp3=False):
    ...
    def _show_video_search_failure_dialog(self, query_text, detail=""):
        message = f"找不到可下載的搜尋結果：\n{query_text}"
        if detail:
            message += f"\n\n{detail}"
        try:
            messagebox.showwarning("搜尋失敗", message, parent=getattr(self, "root", None))
        except Exception:
            write_error_log("video search failure dialog unavailable", Exception(message), query=query_text, detail=detail)

    def _submit_video_search_download_plan(self, plan, parent=None):
        if not plan:
        ...
        self._final_add_download(
            target_url,
            is_mp3=bool(plan.get("is_mp3", False)),
            custom_name=plan.get("custom_name") or None,
            source_site=plan.get("source_site") or None,
            extra_task_data=plan.get("extra_task_data") or None,
        )
        return True

    def _handle_video_search_results(self, query_text, results, is_mp3=False):
        results = list(results or [])
        if not results:
            self._show_video_search_failure_dialog(query_text, "搜尋完成，但沒有找到符合番號/檔名的可下載結果。")
            return
        try:
            self._show_video_search_results_dialog(query_text, results, is_mp3=is_mp3)
        except Exception as exc:
            write_error_log("video search confirmation dialog unavailable", exc, query=query_text)
            plan = self._build_video_search_download_plan(results, 0, query_text, is_mp3=is_mp3)
            if not self._submit_video_search_download_plan(plan):
                self._show_video_search_failure_dialog(query_text, "搜尋完成，但無法建立可下載任務。")

    def _show_video_search_results_dialog(self, query_text, results, is_mp3=False):
        results = list(results or [])
        if not results:
            self._show_video_search_failure_dialog(query_text)
            return
        dialog = tk.Toplevel(self.root)
        dialog.title("確認搜尋結果下載")
        dialog.geometry("940x460")
        ...
        for column, label, width, anchor in (
            ("rank", "#", 46, "center"),
            ("quality", "畫質", 72, "center"),
            ("subtitle", "中文字幕", 88, "center"),
            ("site", "站台", 96, "center"),
            ("title", "檔名 / 標題", 360, "w"),
            ("url", "網址", 260, "w"),
        ):
        ...
        tk.Label(dialog, textvariable=hint_var, anchor="w", justify="left", font=("Microsoft JhengHei UI", 9)).pack(fill="x", padx=12, pady=(4, 8))
        ...
        def confirm_download():
            ...
            plan = self._build_video_search_download_plan(results, index, query_text, is_mp3=is_mp3)
            if not self._submit_video_search_download_plan(plan, parent=dialog):
                return
            dialog.destroy()
    ...
    def _start_video_search_download(self, query_text, is_mp3=False):
        ...
        def worker():
            try:
                ...
                if not results:
                    self._schedule_ui_call(
                        lambda q=query_text: self._handle_video_search_results(q, [], is_mp3=is_mp3)
                    )
                    return
                self._schedule_ui_call(
    ...
    def _log_m3u8_route_selected(self, task, item_id, media_url, source_site=None, fallback_urls=None):
        ...
        write_error_log(
            "m3u8 route selected",
            Exception(f"route={route_label} site={site or 'unknown'}"),
            url=media_url,
            item_id=item_id,
        ...
        for index, candidate_url in enumerate(direct_candidates):
            try:
                if _looks_like_manifest_url(candidate_url):
                    _handoff_manifest(candidate_url, direct_candidates[index + 1 :])
                    return
    ...
    def _download_m3u8_with_ffmpeg(self, item_id, url, save_dir, is_mp3=False, referer="https://www.movieffm.net/", origin="https://www.movieffm.net", _unexpected_retry_count=0, _native_fallback_done=False, _audio_transcode_retry=False):
        ...
        all_manifest_candidates_are_ggjav = bool(candidate_urls) and all(
            _looks_like_manifest_url(candidate) and _ggjav_media_key(candidate)
            for candidate in candidate_urls
        )
        ...
        if not is_mp3:
            ...
            for parallel_index, parallel_url in enumerate(parallel_candidate_urls):
                ...
                try:
                    ...
                    if source_site in ggjav_hls_source_sites:
                        parallel_setup_errors.append((parallel_url, _summarize_log_exception(exc)))
                    else:
                        write_error_log(
                ...
                if parallel_finished:
                    return
            parallel_exhausted_ggjav_candidates = (
                source_site in ggjav_hls_source_sites
                and all_manifest_candidates_are_ggjav
                and bool(parallel_candidate_urls)
            ...
            if source_site in ggjav_hls_source_sites and parallel_setup_errors:
                error_summary = "; ".join(
                    ...
                    for url, message in parallel_setup_errors[:5]
                )
                if len(parallel_setup_errors) > 5:
                    error_summary += f"; +{len(parallel_setup_errors) - 5} more"
                write_error_log(
                    "parallel hls setup candidates exhausted",
                    Exception(error_summary or "parallel HLS setup failed"),
                    url=url,
                    item_id=item_id,
                    source_site=source_site,
                    candidate_count=len(parallel_setup_errors),
                )

        if parallel_exhausted_ggjav_candidates:
            self._set_cached_resolved_link_state(
                task,
                resolved_url="",
                resolved_url_saved_at=0.0,
            ...
            if page_refresh_candidates:
                next_page_url = page_refresh_candidates[0]
                remaining_pages = page_refresh_candidates[1:]
                _set_task_aux_fields(task, page_refresh_candidates=remaining_pages, fallback_urls=remaining_pages)
                self._update_task_state_entry(task, page_refresh_candidates=remaining_pages, fallback_urls=remaining_pages)
                write_error_log(
                    "ggjav hls exhausted page fallback",
                    Exception("GGJAV HLS candidates exhausted; retrying page fallback"),
                    url=url,
                    item_id=item_id,
                    source_site=source_site,
    ...
    def _cached_resolved_link_needs_source_refresh(self, task, cached_resolved_url):
        source_site = _task_source_site_name(task)
        if _is_known_ad_media_url(cached_resolved_url):
            return source_site in ("goodav17", "hohoj")
        ...
        if not normalized_url:
            return False
        host = urllib.parse.urlsplit(normalized_url).netloc.lower()
        if "ggjav.com" not in host:
            return False
        fallback_urls = _dedupe_download_urls(
            _task_field_value(task, "fallback_urls", []),
            primary_url=normalized_url,
        )
        return not fallback_urls

    def _hohoj_task_should_refresh_slow_external_source(self, task, source_url):
        if _task_source_site_name(task) != "hohoj":
            return False
        normalized_source = _normalize_download_url(source_url)
        source_page_url = self._get_task_source_page(task, fallback_url="")
        normalized_source_page = _normalize_download_url(source_page_url)
        if not normalized_source or not normalized_source_page or normalized_source == normalized_source_page:
            return False
        parsed_page = urllib.parse.urlsplit(normalized_source_page)
        if "hohoj.tv" not in parsed_page.netloc.lower() or not parsed_page.path.startswith("/video"):
            return False
        parsed_source = urllib.parse.urlsplit(normalized_source)
        source_host = parsed_source.netloc.lower()
        if "hohoj.tv" in source_host or "ggjav.com" in source_host:
            return False
        if _looks_like_manifest_url(normalized_source):
            return False
        return True
    ...
    def _download_with_cached_resolved_link(self, task, item_id, source_url, cached_resolved_url, save_dir, use_impersonate, is_mp3):
        ...
        if self._cached_resolved_link_needs_source_refresh(task, cached_resolved_url):
            source_page_url = self._get_task_source_page(task, fallback_url=source_url)
            write_error_log(
                "cached media link refresh",
                Exception("cached media link is stale, incomplete, or points to an ad"),
                item_id=item_id,
                source_url=source_url,
            ...
            self._set_task_parse_ui(item_id, message="已記錄連結缺少備援來源，重新分析頁面...")
            self._download_task_internal(source_page_url or source_url, item_id, save_dir, use_impersonate, is_mp3)
            return True
        if _task_source_site_name(task) == "avbebe":
            try:
        ...
        try:
            ...
            if self._should_resume_with_cached_resolved_link(task, url, save_dir, is_mp3):
                ...
                if (
                    ...
                    write_error_log(
                        "hohoj slow external source refresh",
                        Exception("HoHoJ task is on slow external fallback; refreshing source page"),
                        item_id=item_id,
                        source_url=url,
                        source_page=source_page_url,
                        source_site=source_site,
            ...
            detail_message = (
                t("msg_download_error", error=str(exc)[:45])
                if "msg_download_error" in I18N_DICT.get(CURRENT_LANG, {})
                else summarize_error_message(exc, "err_net", 120)
            )
            self._mark_task_error_state(item_id, exc, detail_message)
        finally:
            if has_anime1_lock:
                anime1_dl_lock.release()
            self._schedule_process_queue()
    ...
    def _download_task_internal(self, url, item_id, save_dir, use_impersonate, is_mp3=False):
        ...
        def _prepare_youtube_component_path(component_path, fmt, label):
            if not component_path or not os.path.exists(component_path):
                return
            expected_size = _format_expected_bytes(fmt)
            current_size = self._get_existing_file_size(component_path)
            if current_size <= 0:
                _reset_youtube_component_path(component_path)
                return
            if expected_size > 0 and current_size < expected_size:
                return
            media_info = self._probe_media_info(component_path)
            size_matches = expected_size <= 0 or self._file_size_matches_expected_total(component_path, expected_size, tolerance_ratio=0.02)
            if media_info.get("valid") and size_matches:
                return
            write_error_log(
                "youtube cached component reset",
                Exception(f"YouTube {label} component does not match current media"),
                output=component_path,
                size=current_size,
                expected_total_bytes=expected_size,
                valid=media_info.get("valid"),
        ...
        def _run_youtube_fast_multipart_download(target_url, output_template_stem):
            ...
            if video_fmt and audio_fmt:
                ...
                _prepare_youtube_component_path(video_path, video_fmt, "video")
                _prepare_youtube_component_path(audio_path, audio_fmt, "audio")
                write_error_log(
                    "youtube multipart component download started",
        ...
        def _download_manifest_with_generic_ytdlp(target_url, referer=None, origin=None):
            ...
            try:
                ...
                if self._is_retryable_media_download_error(generic_exc) and _retry_next_page_fallback(
                    "generic manifest download failed; retrying next source",
                    generic_exc,
                ):
                    return True
                raise
        ...
        def _retry_next_page_fallback(reason, exc=None):
            ...
            if fallback_source_site in ("goodav17", "hohoj"):
                fallback_candidates = _filter_secondary_ggjav_media_groups(fallback_candidates)
            fallback_candidates = [
        ...
        def _dispatch_extracted_media_candidates(
            ...
            if prefer_direct and media_url:
                _dispatch_direct_media_download(
                    ...
                    fallback_urls=media_fallback_urls + manifest_candidates + external_page_candidates,
            ...
            if stream_url:
                _dispatch_manifest_download(
                    ...
                    + [candidate for candidate in direct_media_candidates if candidate != stream_url]
                    + external_page_candidates,
                    referer=referer or url,
                    origin=origin,
                    default_route=manifest_default_route,
            ...
            if media_url:
                ...
                return True
            if external_page_candidates:
                external_url = external_page_candidates[0]
                _set_task_identity(
                    name=display_name,
                    source_site=source_site,
                    source_page=source_page or url,
                    fallback_urls=external_page_candidates[1:],
                )
        ...
        if "hohoj.tv" in parsed_url.netloc and parsed_url.path.startswith("/video"):
            ...
            candidate_urls = _expand_ggjav_video_host_fallbacks(
